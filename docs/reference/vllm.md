...
screen -dmS vllm bash -lc 'source ~/.profile && ml && \
  ...
  vllm serve nvidia/NVIDIA-Nemotron-3-Nano-30B-A3B-FP8 \
    ...
    --max-model-len 32768 \
    --reasoning-parser nano_v3 \
    --reasoning-parser-plugin ~/.cache/huggingface/hub/models--nvidia--NVIDIA-Nemotron-3-Nano-30B-A3B-FP8/snapshots/f8dc1c0afee92f44417695b4f5ddca9afc95ea58/nano_v3_reasoning_parser.py \
...
| Nemotron-3 Super 120B NVFP4 (MARLIN) | ~69.5 GiB weights + ~19 GiB KV cache (at `--gpu-memory-utilization 0.75`, `--max-model-len 4096`) |
...
## Nemotron-3-Super-120B-A12B-NVFP4 — ✅ Validated
...
**Confirmed working config:** `--gpu-memory-utilization 0.75` (89 GiB budget → ~19 GiB for KV cache at 4096 context). Validated 2026-03-17: server starts, inference returns correct output, no NaN/OOM errors.

**Reasoning parser:** Super has its own `super_v3_reasoning_parser.py` in the model repo — use `--reasoning-parser super_v3`, not `nano_v3`. Note: the validated launch command below does NOT include the reasoning parser (omitted for initial validation run). Add it for production use.

```bash
# ✅ Validated Super 120B launch command (2026-03-17):
screen -dmS vllm bash -lc 'source ~/.profile && ml && \
  VLLM_USE_FLASHINFER_MOE_FP4=0 VLLM_NVFP4_GEMM_BACKEND=marlin \
  ...
  2>&1 | tee /tmp/vllm-super-075.log'

# With reasoning parser (add for production):
screen -dmS vllm bash -lc 'source ~/.profile && ml && \
  VLLM_USE_FLASHINFER_MOE_FP4=0 VLLM_NVFP4_GEMM_BACKEND=marlin \
  vllm serve nvidia/NVIDIA-Nemotron-3-Super-120B-A12B-NVFP4 \
    --trust-remote-code \
...
**Startup time:** ~8.5 min total (shard loading ~7 min for 17 shards at ~30s each, then CUDA graph capture with cache).
